...
export interface MapCenterStore {
  getMapCenter(): MapCenter | undefined;
  setMapCenter(center: MapCenter): void;
  observeMapCenter(): { subscribe(next: (center: MapCenter) => void): { unsubscribe(): void } };
}

/**
 * Restores the previously stored map center/zoom (if any), persists it on
 * `moveend`, and re-applies it when another tab updates the stored value.
 * Returns a disposer. Replaces the `dragend zoomend` persistence +
 * `observeMapCenter` subscription in the Leaflet map services.
 */
export function bindMapCenterPersistence(map: MlMap, store: MapCenterStore): () => void {
  const saved = store.getMapCenter();
  if (saved) {
