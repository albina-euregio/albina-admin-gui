...
export class SliderComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy, ControlValueAccessor {
  ...
  private updateTicksScale(): void {
    ...
    const tickValueStep: number = !ValueHelper.isNullOrUndefined(this.viewOptions.tickValueStep)
  ...
  private getTicksArray(): number[] {
    const step: number = !ValueHelper.isNullOrUndefined(this.viewOptions.tickStep)
      ? this.viewOptions.tickStep
      : this.viewOptions.step;
  ...
  private bindEvents(): void {
    ...
    if (this.viewOptions.draggableRangeOnly) {
      ...
      if (!this.viewOptions.onlyBindHandles) {
        this.fullBarElement.on("mousedown", (event: MouseEvent): void => this.onStart(null, event, true, true, true));
  ...
  private onKeyboardEvent(event: KeyboardEvent): void {
    ...
    const keyCode: number = !ValueHelper.isNullOrUndefined(event.keyCode) ? event.keyCode : event.which;
