...
function wordDiff(text1: string, text2: string) {
   ...
   * @private
   */
  function diff_linesToChars_(text1: string, text2: string): { chars1: string; chars2: string; lineArray: string[] } {
...
export function createWordDiff(text1, text2) {
  const showDiff = 2; // 0: none, 1: additions only, 2: additions and deletions
  const result: string = wordDiff(text2, text1)
    .map(([diff, value]) =>
